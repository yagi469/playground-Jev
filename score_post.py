...
def verify_post_with_jev(
    ...
    if domain in ["business", "management"]:
        ...
        questions = {
            "mathematical_depth": Score(
                instructions=(
                    "記事中で、核心となる技術的構造（アーキテクチャ、データ構造、アルゴリズム、通信プロトコル、設計パターンなど）が、"
                    "抽象的な説明だけでなく、具体的に論理的に解説されているかを評価してください。"
                ),
                criteria=[
                    "中身が薄い（抽象的なバズワードばかりで、技術的メカニズムが見えない）",
                    "表面的（技術用語は並んでいるが、どういう仕組みで動作・解決するのかが浅い）",
                    "明確で具体的（アーキテクチャ、設計意図、動作メカニズムが明快に解説されている）",
                    "極めて深い（内部構造のトレードオフや設計思想の核心が鮮やかに浮き彫りにされている）",
                ],
            ),
            "pedagogical_clarity": Score(
                instructions=(
            ...
            "critique_diagnosis": Choice(
                ...
                criteria={
                    ...
                    "need_math_details": "技術的メカニズムやアーキテクチャの解説が抽象的。具体的な内部構造の掘り下げが必要",
                    "need_sharp_opinion": "筆者オピニオンが公式ドキュメントの無難なまとめ。設計トレードオフや現場目線のスタンスを熱く語るべき",
                    "avoid_shallow_metaphors": "安易な日常のたとえ話やAI特有のお茶濁しが目立つ。技術の本質的な面白さに徹するべき",
                    "high_quality": "前提の丁寧さ、技術的深さ、オピニオンの鋭さが極めて高い水準で調和している",
                },
            ),
            "lack_of_opinion_risk": Noul(
