...
def verify_post_with_jev(
    ...
    if domain in ["stats", "statistics", "econometrics", "econ", "causal", "統計", "計量経済学"]:
        questions = {
            "mathematical_depth": Score(
                instructions=(
                    "記事中で、核心となる計量経済学・統計的因果推論の枠組み（潜在的結果モデル、識別条件、局所多項式回帰、バンド幅選択、漸近推測など）が、"
                    "表面的なツール紹介ではなく、数理的メカニズムと論理の因果関係を踏まえて掘り下げて解説されているかを評価してください。"
                ),
                criteria=[
                    "中身が薄い（ツールの紹介やバズワードばかりで、識別の数理や仮定のロジックが見えない）",
                    "表面的（数式や用語は並んでいるが、どういうメカニズムで因果効果が同定されるのかの解説が浅い）",
                    "明確で具体的（識別仮定、推定量の性質、バイアス構造のロジックが明快に解説されている）",
                    "極めて深い（ノンパラメトリック理論や推測の数理的本質、実証分析への強力な示唆が鮮やかに浮き彫りにされている）",
                ],
            ),
            "pedagogical_clarity": Score(
                instructions=(
                    "前提となる因果推論の課題（選択バイアス、未観測交絡因子など）や記号の定義が親切に説明され、"
                    "なぜその手法や仮定が必要なのかが、読者が納得できるように明快かつ体系的に解説されているかを評価してください。"
                ),
                criteria=[
                    "読者置いてけぼり（前提や記号の定義がなく、難解な数式や結論だけが唐突に語られている）",
                    "不親切（数式が並んでいるが、なぜその導出になるのかの行間が見えにくい）",
                    "明快で親切（問題の所在から推定・推測のロジックまでが論理的に無理なく追える）",
                    "圧倒的な明快さ（直観的理解と厳密な数理が完璧に調和しており、極めて深く理解できる）",
                ],
            ),
            "author_stance": Score(
                instructions=(
                    "「で、私（筆者）はどう考えるか？」を含め、論文の単なる要約に終始せず、"
                    "実証分析へのインプリケーション、他手法との比較、実務での適用限界や仮定の妥当性など、筆者独自の批判的スタンスが鮮明に打ち出されているかを評価してください。"
                ),
                criteria=[
                    "客観的な論文要約に終始しており、筆者の立場やオピニオンが皆無",
                    "当たり障りのない感想程度で、スタンスが曖昧",
                    "データ分析者・研究者としての着眼点や判断基準が明確に示されている",
                    "強烈な独自オピニオンや実践的洞察があり、知的刺激に満ちている",
                ],
            ),
            "intellectual_appeal": Score(
                instructions=(
                    "計量経済学やデータサイエンスに関心を持つ読者にとって、知的好奇心が強く刺激され、"
                    "「因果推論ってこんなに奥が深くて面白いのか」「自分の分析でも使ってみたい」と思わせる魅力があるかを評価してください。"
                ),
                criteria=[
                    "退屈・安易（教科書の目次のような無味乾燥なまとめで、知的好奇心が湧かない）",
                    "教科書的（正確だが、読者をワクワクさせるような熱量に欠ける）",
                    "魅力的（因果推論の美しさや実証分析の醍醐味が伝わり、読んでいて面白い）",
                    "圧倒的（理論の真の価値と実証研究の深淵が伝わり、読者を強く引き込む名論考）",
                ],
            ),
            "critique_diagnosis": Choice(
                instructions="この記事のクオリティをさらに高めるために、最も改善が必要なボトルネックはどこですか？",
                criteria={
                    "need_pedagogical_steps": "前提知識・記号の定義不足、または数式の途中変形（行間）が省略されている",
                    "need_math_details": "推定量の数理的性質やバイアス構造の解説が抽象的。数理的ロジックの掘り下げが必要",
                    "need_sharp_opinion": "筆者オピニオンが無難なまとめ。実証への示唆や他手法との比較を熱く語るべき",
                    "avoid_shallow_metaphors": "物理学や素粒子論など無関係な分野からの無理なこじつけ、または安易なたとえ話が目立つ。計量経済学・統計学の正統な枠組みに徹するべき",
                    "high_quality": "前提の丁寧さ、数理の深さ、オピニオンの鋭さが極めて高い水準で調和している",
                },
            ),
            "lack_of_opinion_risk": Noul(
                instructions="読者が読み終わった後に「論文の概要は分かったけど、結局筆者はどう考えているの？」と肩透かしを感じるリスクがありますか？"
            ),
            "rushed_math_risk": Noul(
                instructions="説明が抽象的すぎたり前提が飛ばされているために、読者が『置いてけぼり』に感じるリスクがありますか？"
            ),
        }
    elif domain in ["business", "management"]:
        questions = {
            "mathematical_depth": Score(
                instructions=(
    ...
    elif domain in ["classics", "history", "humanities", "philosophy"]:
        questions = {
            "mathematical_depth": Score(
                instructions=(
                    "記事中で、核心となる歴史的背景、登場人物たちの葛藤や生々しい人間ドラマ、勝敗を分けた決断や知略（メーティス）が、"
                    "表面的なあらすじの羅列ではなく、具体的かつ情景が目に浮かぶように掘り下げて解説されているかを評価してください。"
                ),
                criteria=[
                    "中身が薄い（退屈なあらすじの要約だけで、人間ドラマや極限状態での葛藤が見えない）",
                    "表面的（エピソードは並んでいるが、登場人物の選択の背景や心理的・戦略的ロジックが浅い）",
                    "明確で具体的（過酷な状況、勝敗を分けた決断や知略のロジックが生き生きと解説されている）",
                    "極めて深い（古典が描く人間性の深淵、極限のリーダーシップや知略の核心が鮮やかに浮き彫りにされている）",
                ],
            ),
            "pedagogical_clarity": Score(
                instructions=(
                    "前提となる時代背景や神話的世界観、人物相関が親切に説明され、専門知識がない読者でも"
                    "物語の展開や登場人物の動機が直観的かつ自然に理解できるように解説されているかを評価してください。"
                ),
                criteria=[
                    "読者置いてけぼり（登場人物や背景の説明がなく、話の筋が唐突に語られている）",
                    "文脈が不親切（固有名詞が並んでいるが、なぜその行動に至ったのかの因果関係が見えにくい）",
                    "明快で親切（背景からドラマの展開までが無理なく自然に追える）",
                    "圧倒的な語り口（情景が鮮やかに目に浮かび、数千年前の物語に完全に引き込まれる）",
                ],
            ),
            "author_stance": Score(
                instructions=(
                    "「で、私（筆者）はどう考えるか？」を含め、単なる古典の解説・あらすじ紹介に終始せず、"
                    "現代のビジネス、組織運営、キャリア、誘惑への対処（コミットメント）、不確実性下の意思決定など、"
                    "筆者独自の現代的な批評眼・スタンスが鮮明に打ち出されているかを評価してください。"
                ),
                criteria=[
                    "客観的な昔話の要約に終始しており、筆者の現代的な視点やオピニオンが皆無",
                    "当たり障りのない教訓程度で、独自のスタンスが曖昧",
                    "現代の社会や仕事に引きつけた独自の着眼点や問いが明確に示されている",
                    "強烈な独自オピニオンや鋭い現代的批評があり、極めて深い知的刺激に満ちている",
                ],
            ),
            "intellectual_appeal": Score(
                instructions=(
                    "教養、人間心理、戦略・思考法に関心を持つ読者にとって、知的好奇心が強く刺激され、"
                    "「古典ってこんなに実践的で面白いのか」「自分も読んでみたい」と思わせる魅力があるかを評価してください。"
                ),
                criteria=[
                    "退屈・安易（学校の教科書のような無味乾燥なまとめで、知的好奇心が湧かない）",
                    "教科書的（整ってはいるが、読者の心を揺さぶる熱量や現代へのフックに欠ける）",
                    "魅力的（古典の人間ドラマと現代的意義が鮮やかに伝わり、読んでいて面白い）",
                    "圧倒的（古典の普遍的な知恵とスリルが現代に蘇り、読者の世界観を揺さぶる名論考）",
                ],
            ),
            "critique_diagnosis": Choice(
                instructions="この記事のクオリティをさらに高めるために、最も改善が必要なボトルネックはどこですか？",
                criteria={
                    "need_pedagogical_steps": "時代背景や登場人物の関係性の説明が不足し、読者が情景を思い浮かべにくい",
                    "need_math_details": "人間ドラマや決断の背景が抽象的。核心エピソードの具体的な心理・知略の描写が必要",
                    "need_sharp_opinion": "筆者オピニオンが無難な昔話の感想。現代のキャリアや意思決定への鋭い示唆を熱く語るべき",
                    "avoid_shallow_metaphors": "無関係なIT用語や数式の無理なこじつけ、またはAI特有のお茶濁しが目立つ。人間ドラマと古典の知恵に徹するべき",
                    "high_quality": "情景の鮮やかさ、エピソードの深さ、現代的オピニオンの鋭さが極めて高い水準で調和している",
                },
            ),
            "lack_of_opinion_risk": Noul(
                instructions="読者が読み終わった後に「昔話の筋は分かったけど、結局筆者は現代の視点からどう考えているの？」と肩透かしを感じるリスクがありますか？"
            ),
            "rushed_math_risk": Noul(
                instructions="固有名詞や展開が唐突で、読者が『置いてけぼり』に感じるリスクがありますか？"
            ),
        }
    else:
    ...
    questions = {
        "genre": Choice(
            ...
            criteria={
                ...
                "classics": (
                    "古代ギリシャ・ローマ古典、東洋・西洋の歴史書、英雄譚・叙事詩、神話、"
                    "古典文学、哲学思想、文明論などの歴史・人文学に関する古典的名著"
                ),
                "general": (
                    "社会科学、文化、認知科学、一般教養、その他の一般的な論考や書籍"
                ),
            },
        )
    }
