...
{{
  ...
  "genre": "stats（統計学・計量経済学・因果推論・データサイエンス）, business（ビジネス・マネジメント・組織論・経済）, tech（ソフトウェア・システム設計・工学）, physics（数理物理・理論物理・量子・科学論文）, classics（歴史・古典文学・哲学・英雄譚・思想書）, general（一般教養・その他）のいずれか1つを必ず選択",
  ...
  "slug": "このドキュメントまたは対象セクションを表す、URLやファイル名に最適な英小文字とハイフンのみの短いスラッグ（例: 'homeros-odysseia-1', 'high-output-management-ch1', 'cashflow-quadrant-ch4'）。日本語や記号を含めず、簡潔で分かりやすい英語表記にしてください。"
}}
"""
        try:
            res = client.models.generate_content(
...
{{
  ...
  "slug": "このドキュメントまたは対象セクションを表す、URLやファイル名に最適な英小文字とハイフンのみの短いスラッグ（例: 'writing-for-developers-ch1'）。日本語や記号を含めず、簡潔で分かりやすい英語表記にしてください。"
}}
