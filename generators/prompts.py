...
"""
    ...
    elif genre in ["stats", "statistics", "econometrics", "econ", "causal", "統計", "計量経済学"] or any(
        k in subfield or k in title_lower
        for k in ["econometrics", "causal", "regression discontinuity", "rdd", "treatment effect", "統計学", "計量経済学", "因果推論", "回帰分析"]
    ):
        persona = (
            "あなたは計量経済学、統計的因果推論、ノンパラメトリック推定の最前線を探究する、気鋭の計量経済学者兼データサイエンスエッセイストです。\n"
            "単なる入門書の薄い要約やツールのマニュアル紹介に逃げず、経済学・因果推論が格闘してきた『識別の壁』『選択バイアスの本質』『推定と推測（Inference）の数理』を生き生きと語り尽くす、知的好奇心と実証研究への深い洞察に満ちたブログ記事を執筆してください。"
        )
        sections = f"""
     - ## 導入（1行サマリー ＆ つかみ）: この記事でわかることと読者の知的好奇心を一気に引き込む導入。冒頭で対象論文・ドキュメント（『{title}』{ch_str}）に言及し、因果推論・実証分析における核心的ブレイクスルーと問題意識を1分で掴むロードマップを示してください。
     - ## 識別の壁と因果推論の課題: なぜ単純な比較や相関分析では破綻するのか、背後にある選択バイアスや未観測交絡因子の構造を論理的かつクリアに解説。
     - ## 核心理論と数理的メカニズム: 著者が展開する識別枠組み（Identification Framework）、推定量（Estimator）、漸近推測（Inference）、検証手法（Falsification/Validation）を、数式と直観を結びつけて体系的に解説。
     - ## で、私（筆者）はどう考えるか？: （★最重要：実証・理論の視点からの独自オピニオン）単なる要約で終わらせず、実証分析へのインプリケーション、他手法（DID、操作変数法、機械学習アプローチ等）との比較、実務での適用限界や注意点について、骨太なオピニオンを展開。
     - ## まとめ ＆ 参考文献・実務指針: 記事の総括と、実務家・研究者が押さえるべき重要教訓と文献リスト。
"""
        guidance = (
            "- 【最重要：無理な物理比喩・こじつけの禁止】素粒子論や場の量子論、量子力学の比喩（境界欠陥、ゲージ対称性、有効場の理論、BCFTなど）を絶対に持ち出さないでください。\n"
            "- 計量経済学・統計学の正統かつ厳密な枠組み（潜在的結果、局所的無作為化、連続性仮定、バンド幅選択、バイアス修正信頼区間など）に基づき論理的に解説してください。\n"
            "- 【強調タグのルール】Markdownの ** 記法ではなく、HTMLの <strong> タグ（例: <strong>太字テキスト</strong>）を使用してください。\n"
            "- 独立行の数式ブロック（$$ ... $$）を適切に活用し、途中計算の行間を丁寧に補ってください。"
        )
        default_tags = ["計量経済学", "因果推論", "統計学", "データサイエンス"]
